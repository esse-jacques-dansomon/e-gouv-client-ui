...
const Header = () => {
  ...
  return (
    <header className=" bg-white  fixed top-0 left-0 right-0 z-30 ">
      <div className="bg-white text-white ">
        <div className="app-container flex items-center justify-between">
          <div className="flex items-center h-8 justify-between text-sm">
            <Link
              href="/"
              className={`hover:underline px-4 text-center h-8  flex items-center justify-center ${
                !isEspaceEntreprise ? "bg-primary" : "bg-white text-black "
              }`}>
              Espace Citoyen
            </Link>
            <Link
              href="/entreprise"
              className={`hover:underline px-4 text-center h-8  flex items-center justify-center ${
                isEspaceEntreprise ? "bg-senRed" : "bg-white text-black "
              }`}>
              Espace Entreprise
            </Link>
          </div>
          <div className="space-x-2  sm:flex text-sm">
            <form className="max-w-sm mx-auto">
              <select
                ...
                id="countries"
                className="bg-white  text-primary border-white block w-full h-8 px-2 ">
        ...
        <div className="app-container flex justify-between items-center py-3 border-b-2 border-white">
          <div className="flex items-center justify-center">
            <div
              onClick={toggleMenu}
              id="menu-btn"
              className="text-gray-800 font-bold flex space-x-2 ">
              <Image
          ...
          <div className="flex items-center justify-between  text-white text-sm gap-1">
            ...
            <Link
              href="/dashboard"
              className="hover:underline px-3 py-1 bg-sendGreen">
              Mon Compte
            </Link>
          </div>
        </div>
      ...
      <div
        id="menu"
        className={` bg-white absolute top-0 left-0 right-0 z-10 overflow-y-scroll min-h-[100vh] max-w-[540px] shadow-lg ${
          isOpen ? "block" : "hidden"
        }`}>
        ...
        <div className="grid grid-cols-1 divide-gray-50 px-4 py-4 gap-y-4">
          ...
          <div className="">
            ...
            <div
              className={`flex flex-col space-y-4 pl-3 mt-2 ${
                isMenuOpen ? "block" : "hidden"
              }`}>
              <Link
                onClick={toggleMenu}
                href="/services/education-formation"
                className="hover:underline">
                Education & Formation
              </Link>
              <Link
                onClick={toggleMenu}
                href="/services/emploi-securite-sociale"
                className="hover:underline">
                Emploi & Sécurité sociale
              </Link>
              <Link
                onClick={toggleMenu}
                href="/services/papiers-citoyennete"
                className="hover:underline">
                Papiers & Citoyenneté
              </Link>
              <Link
                onClick={toggleMenu}
                href="/services/fiscalite-foncier-douanes"
                className="hover:underline">
                Fiscalité, Foncier & Douanes
              </Link>
              <Link
                onClick={toggleMenu}
                href="/services/agriculture-elevage-industrie"
                className="hover:underline">
                Agriculture, Elevage & Industrie
              </Link>
              <Link
                onClick={toggleMenu}
                href="/services/sante-protection-sociale"
                className="hover:underline">
                Santé & Protection sociale
              </Link>
              <Link
                onClick={toggleMenu}
                href="/services/loisirs-media-culture"
                className="hover:underline">
                Loisirs, Media & Culture
              </Link>
              <Link
                onClick={toggleMenu}
                href="/services/habitat-transport"
                className="hover:underline">
                Habitat & Transport
              </Link>
              <Link
          ...
          <div className="flex flex-col justify-start items-start gap-3">
            <Link
              href="/auth/login"
              className="bg-white border-green-500 border-2 text-w text-sm px-2 py-2 rounded">
              Se connecter
            </Link>
            <Link
              href="/auth/register"
